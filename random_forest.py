...
BASE_DIR = WORKSPACE_DIR / "Raman_spectral_signature_analysis" / "Raman_Data"
EXCEL_PATH = BASE_DIR / "clinical characters of patients.xls"
COMMON_AXIS = np.linspace(400, 1800, 700)
SG_WINDOW = 9
SG_POLY = 3
N_SPLITS = 5
RANDOM_STATE = 42
VAL_PATIENT_RATIO = 0.2
...
def build_dataframe(base_dir, clinical_dict):
    ...
    for folder in all_folders:
        ...
        for txt_file in folder.glob("*.txt"):
            raw_pid = extract_patient_id(txt_file.name)
...
def preprocess_spectrum(filepath, common_axis=COMMON_AXIS, sg_window=SG_WINDOW, sg_poly=SG_POLY):
    ...
    area = np.trapezoid(spectrum, common_axis)
    if area > 0:
...
def run_cross_validation(X, y, patient_ids, n_splits=5, val_patient_ratio=VAL_PATIENT_RATIO):
    ...
    for train_idx, test_idx in sgkf.split(X, y, groups=patient_ids):
        ...
        unique_labels = y_train_val[pid_indices]
        ...
        spec_acc = accuracy_score(y_test, y_pred_test)
        spec_auc = roc_auc_score(y_test, y_prob_test)
        tn, fp, fn, tp = confusion_matrix(y_test, y_pred_test).ravel()
        spec_sens = tp / (tp + fn) if (tp + fn) > 0 else 0.0
        spec_spec = tn / (tn + fp) if (tn + fp) > 0 else 0.0

        test_df = pd.DataFrame({"patient_id": test_pids, "true": y_test, "prob": y_prob_test})
        ...
        fold_results.append({
            ...
            "val_auc": roc_auc_score(y_val_final, y_prob_val),
...
if __name__ == "__main__":
    ...
    if results:
        last_fold = results[-1]
        val_auc_values = [r["val_auc"] for r in results]
        folds = np.arange(1, len(results) + 1)
        importance_values = last_fold["feature_importance"]
        top_idx = np.argsort(importance_values)[-20:]
        top_importance = importance_values[top_idx]
        top_wavenum = COMMON_AXIS[top_idx]

        plt.figure(figsize=(12, 4))
        plt.subplot(1, 2, 1)
        plt.plot(folds, val_auc_values, marker='o', label='Validation AUC')
        plt.axhline(np.mean(val_auc_values), color='r', linestyle='--', label='Mean AUC')
        plt.title('Fold Validation AUC')
        plt.xlabel('Fold')
        plt.ylabel('AUC')
        plt.legend()

        plt.subplot(1, 2, 2)
        plt.plot(top_wavenum, top_importance, marker='o')
